...
const Profile = () => {
  ...
  return (
    <div className="max-w-md mx-auto p-6 bg-white rounded shadow">
      ...
      <h2 className="text-center text-xl font-bold mt-4 text-black">{displayName}</h2>
      <p className="text-center text-black">{user?.email}</p>
      <p className="text-center text-black">Role: {dbUser?.role || "student"}</p>
      <p className="text-center mt-2 text-black">Phone: {dbUser?.phone || "Not Provided"}</p>
    </div>
  );
};
